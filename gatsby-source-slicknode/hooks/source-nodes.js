"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.sourceNodes = void 0;
var graphql_1 = require("graphql");
var relay_pagination_1 = require("./pagination/relay-pagination");
var gatsby_graphql_source_toolkit_1 = require("gatsby-graphql-source-toolkit");
function getRootListField(type) {
    var typeName = type.name;
    var parts = typeName.split('_');
    if (parts.length > 1) {
        return parts.shift() + '_list' + parts.join('_');
    }
    else {
        return "list" + typeName;
    }
}
var IGNORED_TYPES = [
    'ContentNode',
    'File',
    'Image',
];
function createSourcingConfig(gatsbyApi, pluginOptions) {
    return __awaiter(this, void 0, void 0, function () {
        var preview, endpoint, typePrefix, execute, schema, types, gatsbyNodeTypes, fragments, documents;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    preview = pluginOptions.preview, endpoint = pluginOptions.endpoint, typePrefix = pluginOptions.typePrefix;
                    if (!endpoint) {
                        gatsbyApi.reporter.panic('gatsby-source-slicknode: Plugin option endpoint not configured');
                    }
                    execute = gatsby_graphql_source_toolkit_1.createDefaultQueryExecutor(String(endpoint), {
                        headers: {
                            'x-slicknode-preview': preview ? '1' : '0',
                        },
                    });
                    return [4 /*yield*/, gatsby_graphql_source_toolkit_1.loadSchema(execute)];
                case 1:
                    schema = _a.sent();
                    types = schema
                        .getPossibleTypes(graphql_1.assertInterfaceType(schema.getType('Node')))
                        .filter(function (type) { return (
                    // Ignored types
                    !IGNORED_TYPES.includes(type.name) &&
                        // Do not include version types
                        !type.name.endsWith('_Version')); });
                    gatsbyNodeTypes = types.map(function (type) { return ({
                        remoteTypeName: type.name,
                        queries: "\n      query LIST_" + type.name.toUpperCase() + "($first: Int, $after: String) {\n        " + getRootListField(type) + "(first: $first, after: $after){\n          edges {\n            cursor\n            node {\n              ..._" + type.name + "Id_\n            }\n          }\n        }\n      }\n      fragment _" + type.name + "Id_ on " + type.name + " { __typename id }\n    ",
                    }); });
                    console.log('Types', gatsbyNodeTypes);
                    fragments = gatsby_graphql_source_toolkit_1.generateDefaultFragments({ schema: schema, gatsbyNodeTypes: gatsbyNodeTypes });
                    console.log('Fragments', fragments);
                    documents = gatsby_graphql_source_toolkit_1.compileNodeQueries({
                        schema: schema,
                        gatsbyNodeTypes: gatsbyNodeTypes,
                        customFragments: fragments,
                    });
                    return [2 /*return*/, {
                            gatsbyApi: gatsbyApi,
                            schema: schema,
                            execute: execute,
                            paginationAdapters: [
                                relay_pagination_1.RelayPagination,
                            ],
                            gatsbyTypePrefix: "Example",
                            gatsbyNodeDefs: gatsby_graphql_source_toolkit_1.buildNodeDefinitions({ gatsbyNodeTypes: gatsbyNodeTypes, documents: documents }),
                        }];
            }
        });
    });
}
function sourceNodes(gatsbyApi, options) {
    return __awaiter(this, void 0, void 0, function () {
        var config;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, createSourcingConfig(gatsbyApi, options)];
                case 1:
                    config = _a.sent();
                    // Step5. Add explicit types to gatsby schema
                    return [4 /*yield*/, gatsby_graphql_source_toolkit_1.createSchemaCustomization(config)];
                case 2:
                    // Step5. Add explicit types to gatsby schema
                    _a.sent();
                    // Step6. Source nodes
                    return [4 /*yield*/, gatsby_graphql_source_toolkit_1.sourceAllNodes(config)];
                case 3:
                    // Step6. Source nodes
                    _a.sent();
                    return [2 /*return*/];
            }
        });
    });
}
exports.sourceNodes = sourceNodes;
